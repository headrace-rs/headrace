...
- Not infrastructure, a layer. No broker, no storage engine, no custom controller - rent those.
...
```mermaid
flowchart TB
  subgraph mono[Monolithic - dev / edge]
...
The `Backend` trait is the boundary (`crates/headrace-core/src/backend.rs`): `producer(id) ->
...
- Server-side `partition(P, ...)` subject transform hashes the group key to a partition index `0..P-1` on the subject.
...
- **Authoring API:** the v0.2 **MCP server** is how an agent creates a dataflow - emit IR ->
  validate against the schema -> dry-run -> write it as a file or CR. No custom REST surface.
...
headrace records its own metrics through a `Metrics` boundary (`headrace-core::metrics`) - default
