...
//! window, emitting one record per non-empty group at each boundary. Stateful.
//!
//! [`Window`] is the pure, synchronous core (fold + flush); [`run`] is the async driver
//! that owns only timing and I/O.
...
/// The async driver ([`run`]) owns only timing and I/O.
pub struct Window {
    group_by: Vec<String>,
    aggregate: Aggregate,
...
/// Drive the window on a tumbling timer: fold records until each boundary, flush, repeat.
